...
substance_patterns = {
    ...
    'alucinogenos': (
        r'\blsd\b|\btusi\b|\b2cb\b|\bcandy fly\b|\btrip\b|\blucy\b|\bpapel\b|'
...
1) ALUCINOGENOS
   ...
   - LSD / ácido lisérgico / “ácido” claramente relacionado con LSD
   - hongos psilocibios / "hongos alucinogenos"
   - mescalina, peyote
   - tusi, 2CB, MDMA, “molly”, “éxtasis”
   - ketamina usada como droga recreativa
   - borrachero, floripondio, cacao sabanero, burundanga, etc., como sustancias alucinógenas.
...
2) ESTIMULANTES
   ...
   - metilfenidato, Adderall, Concerta
   - MDMA (también puede ser "alucinogenos" si aplica)
   - cocaína usada como estimulante (además de "cocaina_y_derivados")
...
3) INHALANTES
   ...
   - pegantes / solventes volátiles: thinner, sacol, boxer, pegante,
     disolventes de pintura, varsol, gasolina inhalada, etc.
   ...
   - gases de uso doméstico o industrial:
       "gas propano", "gas natural", "gas metano",
       "gas domiciliario", "gases fluorados",
       "dioxido de carbono", "monoxido de carbono",
       "gas de mina de carbon", etc.
...
4) TRANQUILIZANTES_Y_SEDANTES
   Incluye:
   - benzodiacepinas (clonazepam, alprazolam, diazepam, lorazepam, etc.)
   - hipnóticos tipo z (zopiclona, zolpidem)
   - antipsicóticos, antiepilépticos y moduladores del ánimo
     con potencial de abuso sedante (quetiapina, olanzapina, haloperidol,
     lamotrigina, acido valproico, barbituricos, etc.)
...
5) ALCOHOL_ETANOL
   - Bebidas alcohólicas y preparaciones donde el alcohol etílico es
     claramente la sustancia: "cerveza", "vino", "aguardiente",
     "ron", "whisky", "vodka", "four loko", etc.

6) CANNABINOIDES, COCAINA_Y_DERIVADOS, OPIOIDES, ESCOPOLAMINA
   - marihuana, cannabis, bareto, cripa, THC, hashish → "cannabinoides"
   - cocaína, perico, bazuco, crack, base de coca → "cocaina_y_derivados"
   - heroína, fentanilo, morfina, tramadol, codeína, opioides en general → "opioides"
...
def clasificar_sustancias_con_llm_batch(nombres_pro_lista: List[str]) -> Dict[str, List[str]]:
    """
    Usa exclusivamente el LLM para clasificar las sustancias en lotes.
    Añade:
      - IDs por ítem para evitar problemas de mapeo
      - Chequeos de cobertura (qué ítems no fueron respondidos por el LLM)
    """
